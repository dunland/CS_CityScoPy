...
class Cityscopy:
    ...
    def __init__(self, path):
        ...
        pipeline = rs.pipeline()  # The pipeline streaming loop captures samples from the device, and delivers them to the attached computer vision modules and processing blocks, according to each module requirements and threading model. During the loop execution, the application can access the camera streams by calling wait_for_frames() or poll_for_frames(). The streaming loop runs until the pipeline is stopped.
        config = rs.config()
        image_width = 640
        image_height = 480
    ...
    def scanner_function(self, multiprocess_shared_dict):
        ...
        if grid_dimensions_y < grid_dimensions_x:
            ...
            video_resolution_x = int(image_width)
            # video_resolution_y = int(video_capture.get(3)*grid_ratio)
            video_resolution_y = int(image_width*grid_ratio)
        else:
            # get the smaller of two grid ratio x/y or y/x
            grid_ratio = grid_dimensions_x / grid_dimensions_y
            video_resolution_x = int(image_width*grid_ratio)
            video_resolution_y = int(image_width)

            # video_resolution_y = int(video_capture.get(3))

        # Show images
        cv2.namedWindow('RealSense', cv2.WINDOW_AUTOSIZE)
        cv2.moveWindow('RealSense', 10, 100)
    ...
    # run the video loop forever
        while True:
            ...
            if self.table_settings['gui'] is True:
                ...
                cv2.imshow('RealSense', keystoned_video)
                cv2.waitKey(1)
